...
DEFAULT_PROMPT = """请根据以下要求生成 20 个用户登录功能测试用例：
...
def handle_generate(username_format: str, password_format: str,
                    model: str, custom_prompt: str) -> tuple[str, str | None, str]:
    """处理「生成测试用例」按钮点击。"""
    if not _ENV_API_KEY:
        return "", None, "⚠️ 请先在 test/day1/.env 中设置 DEEPSEEK_API_KEY"
    ...
    try:
        ...
        return markdown_text, str(excel_path), f"✅ 用例生成完成 → {excel_path}"
